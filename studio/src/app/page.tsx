...
async function getStats() {
  try {
    ...
    const mp4Files = fs.existsSync(exportDir)
      ? fs.readdirSync(exportDir).filter((f) => f.endsWith(".mp4"))
      : [];
    ...
    return {
      totalGames: Math.max(mp4Files.length, logFiles.length),
      totalBatches: Math.max(1, Math.ceil(queueFiles.length / 10)),
      doneGames: mp4Files.length,
      failedGames: 0,
    };
  } catch {
