...
function StudioContent() {
  ...
  const mechanicConfig: Record<
    Mechanic,
    { label: string; icon: string; min: number; max: number; color: string; badge?: string }
  > = {
    GUESS_THE_PRICE: {
      label: "Đoán Giá (G9)",
      icon: "🎯",
    ...
    ODD_ONE_OUT: { label: "Kẻ Lạ Trong Bầy", icon: "🔍", min: 4, max: 4, color: "border-amber-500" },
  };
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                <label className="text-gray-400 text-xs font-medium uppercase tracking-widest block">
                  Mechanic
                </label>
              ...
              {/* Quick Select P0 Flagships */}
              <div className="mb-4 p-3 bg-gradient-to-r from-emerald-950/40 via-teal-950/40 to-rose-950/40 rounded-xl border border-gray-700/60">
                <div className="text-[11px] font-semibold text-gray-300 mb-2 flex items-center gap-1.5">
                  <span>⭐</span>
                  <span className="uppercase tracking-wider">Quick Select P0 Flagships:</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { m: "GUESS_THE_PRICE" as Mechanic, name: "G9: Đoán Giá", icon: "🎯", color: "hover:border-emerald-500 hover:text-emerald-300" },
                    { m: "GROCERY_BASKET" as Mechanic, name: "G7: Siêu Thị", icon: "🛒", color: "hover:border-teal-500 hover:text-teal-300" },
                    { m: "DEAL_OR_SCAM" as Mechanic, name: "G41: Deal/Scam", icon: "🏷️", color: "hover:border-rose-500 hover:text-rose-300" },
                  ].map(({ m, name, icon, color }) => (
                    <button
                      key={m}
                      type="button"
                      onClick={() => {
                      ...
                      className={`py-2 px-2.5 rounded-lg text-xs font-bold border transition-all flex items-center justify-center gap-1.5 ${
                        mechanic === m
                          ...
                          : `bg-gray-800/80 border-gray-700 text-gray-300 ${color}`
                      }`}
                    >
                  ...
                  ([m, cfg]) => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-center transition-all relative ${
                        mechanic === m
                          ? `${cfg.color} bg-gray-800 text-white shadow-md`
                          : "border-gray-700 bg-gray-800/50 text-gray-400 hover:text-white hover:border-gray-600"
                      }`}
                    >
                      {cfg.badge && (
                        <span className="absolute -top-1.5 -right-1.5 text-[8px] font-bold bg-indigo-500 text-white px-1.5 py-0.2 rounded-full uppercase">
                          {cfg.badge}
                        </span>
                      )}
                      <div className="text-xl mb-1">{cfg.icon}</div>
                      <div className="text-xs font-medium leading-tight truncate">{cfg.label}</div>
                      <div className="text-[10px] font-mono text-gray-500 mt-0.5 truncate">{m}</div>
                    </button>
                  )
                )}
              </div>
