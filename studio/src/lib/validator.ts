...
export function validateSchema(game: Partial<GameJson>): ValidationError[] {
  ...
  if (!game.scenes || !Array.isArray(game.scenes) || game.scenes.length === 0) {
    ...
    // Duration total: 15-21s (standard) or 35-42s (multi-round)
    const totalDuration = game.scenes.reduce((sum, s) => sum + (s.duration || 0), 0);
    const isDurationValid =
      (totalDuration >= 14.5 && totalDuration <= 21.5) ||
      (totalDuration >= 35.0 && totalDuration <= 42.0);
    if (!isDurationValid) {
      errors.push({
        code: "E_SCHEMA_SCENE_INVALID",
        field: "scenes",
        hint: `Total duration ${totalDuration.toFixed(1)}s must be 15-21s (standard) or 35-42s (multi-round)`,
      });
    }
...
export function validateGameLogic(game: Partial<GameJson>): ValidationError[] {
  ...
  if (mechanic === "GUESS_THE_PRICE") {
    ...
    if (!gp.answer) {
      errors.push({
        code: "E_GAME_LOGIC_INVALID",
        field: "gameplay.answer",
        hint: "GUESS_THE_PRICE requires an answer",
      });
    }
  }
  ...
  if (mechanic === "GROCERY_BASKET") {
    ...
    if (!gp.productIds || gp.productIds.length !== 3) {
      errors.push({
        code: "E_GAME_LOGIC_INVALID",
        field: "gameplay.productIds",
        hint: "GROCERY_BASKET requires exactly 3 productIds",
      });
    }
    if (!["under", "over"].includes(gp.answer)) {
      errors.push({
        code: "E_GAME_LOGIC_INVALID",
        field: "gameplay.answer",
        hint: "GROCERY_BASKET answer must be 'under' or 'over'",
      });
    }
  }
  ...
  if (mechanic === "DEAL_OR_SCAM") {
    ...
    if (!["deal", "scam"].includes(gp.answer)) {
      errors.push({
        code: "E_GAME_LOGIC_INVALID",
        field: "gameplay.answer",
        hint: "DEAL_OR_SCAM answer must be 'deal' or 'scam'",
      });
    }
  }
