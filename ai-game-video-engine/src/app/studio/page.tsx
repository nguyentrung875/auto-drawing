...
function StudioContent() {
  ...
  async function handleRenderMp4() {
    ...
    setError(null);
    try {
      // First ensure game JSON is generated and displayed
      ...
      if (gameRes.ok && gameData.game) {
        ...
        setWarnings(gameData.warnings || []);
        setPlanningMs(gameData.planningMs || 0);
      }

      // Trigger high-fidelity browser render pipeline
      ...
      if (!res.ok) {
        throw new Error(data.error || data.stderr || "Render MP4 failed");
      }

      setVideoUrl(data.videoUrl);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Render MP4 failed");
    } finally {
            ...
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  disabled={!canRender || loading || renderingMp4}
                  className={`flex-1 py-4 rounded-xl font-bold text-base transition-all flex items-center justify-center gap-2 ${
                    canRender && !loading && !renderingMp4
                      ? "bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:opacity-95 text-white hover:scale-[1.01] shadow-xl shadow-indigo-900/40"
                ...
                <button
                  ...
                  disabled={!canRender || loading || renderingMp4}
                  className="px-5 py-4 rounded-xl font-semibold text-sm bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white transition-all border border-gray-700 disabled:opacity-50"
                >
                  {loading ? "Đang tính..." : "⚡ Xem Kịch Bản"}
                </button>
              </div>

              {renderingMp4 && (
                <div className="bg-indigo-950/70 border border-indigo-500/40 rounded-xl p-4 flex items-center gap-3">
                  <span className="w-5 h-5 border-2 border-indigo-400/30 border-t-indigo-400 rounded-full animate-spin flex-shrink-0" />
                  <div className="text-xs text-indigo-200">
                    <div className="font-semibold text-white">Đang render video 1080×1920 qua Chromium Headless & FFmpeg...</div>
                    <div className="text-indigo-400 mt-0.5">Hệ thống đang chụp 540 khung hình mượt mà và phối nhạc SFX (khoảng 15-25 giây).</div>
                  </div>
                </div>
              )}
            </div>
