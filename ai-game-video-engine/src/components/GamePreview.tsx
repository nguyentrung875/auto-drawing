...
export function GamePreview({ game, planningMs, videoUrl, onRenderMp4, isRendering }: GamePreviewProps) {
  const [activeTab, setActiveTab] = useState<"video" | "preview" | "json" | "timeline" | "audio">(
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
      ...
      <div className="border-b border-gray-800 p-4 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-green-400 text-xs font-medium">✅ Game Kịch Bản</span>
        ...
        <div className="flex items-center gap-2">
          {onRenderMp4 && (
            <button
              ...
              className="text-xs px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold transition-all shadow-md shadow-indigo-900/30"
            >
              {isRendering ? "⏳ Đang Render..." : "🎬 Render MP4"}
            </button>
          )}
          <button
            onClick={copyJson}
      ...
      <div className="border-b border-gray-800 flex">
        {(["video", "preview", "json", "timeline", "audio"] as const).map((tab) => (
          <button
            key={tab}
          ...
          <div className="flex flex-col items-center gap-4 py-2">
            {videoUrl ? (
              <div className="w-full flex flex-col items-center">
                <div className="relative rounded-2xl overflow-hidden shadow-2xl border-2 border-indigo-500/40 bg-black max-w-[280px]">
                  <video
                    controls
                    autoPlay
                    playsInline
                ...
                <div className="flex items-center gap-3 mt-4">
                  <a
                    href={videoUrl}
                    download={`${game.metadata.gameId}.mp4`}
                    className="px-4 py-2 rounded-xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold transition-all shadow-lg shadow-green-900/40 flex items-center gap-1.5"
                  >
                    ⬇️ Tải Video MP4
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3.5 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-medium transition-colors border border-gray-700"
                  >
                    Mở tab mới ↗
                  </a>
                </div>
              ...
              <div className="text-center py-12 px-6 border-2 border-dashed border-gray-800 rounded-xl w-full max-w-sm">
                <div className="text-4xl mb-3">🎬</div>
                <div className="text-sm font-semibold text-white mb-1">Chưa có file MP4</div>
                <p className="text-xs text-gray-400 mb-4">
                  Bấm nút dưới đây để render video 1080×1920 qua Chromium Headless & FFmpeg.
                </p>
                {onRenderMp4 && (
                  <button
                    onClick={onRenderMp4}
                    disabled={isRendering}
                    className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold transition-all shadow-lg shadow-indigo-900/40"
                  >
                    {isRendering ? "⏳ Đang render..." : "🚀 Bắt đầu Render MP4"}
                  </button>
                )}
              </div>
            )}
